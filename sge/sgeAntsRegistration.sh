...
outdir="/data/vision/polina/scratch/patchRegistration/output/${datatype}/ANTs_v526_brainpad10_ds7us7reg_noaffine_multiparam_backward/"
mkdir -p $outdir
...
do
  ...
  do
    ...
    coreName="${datatype}61-2-${subjname}" # base of name for ants output
    outfilecore="${subjpath}/final/${datatype}61-2-${subjname}-"
    outfileSeg="${subjpath}/final/${datatype}61-seg-in-${subjname}_via_${coreName}-warp.nii.gz"
    outfileSegRaw="${subjpath}/final/${datatype}61-seg-in-${subjname}-raw_via_${coreName}-warp.nii.gz"
    #  setup commands
