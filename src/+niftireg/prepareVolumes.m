    ...
    if ~doload
        ...
        if dosparse
            ...
            if isfield(paths, 'sourceMaskFile') && ~isempty(paths.sourceMaskFile)
                sourceMask = niftireg.prepNiiToVol(paths.sourceMaskFile, volPad, opts.maxVolSize) > 0;
            else
            ...
            if isfield(paths, 'targetMaskFile') && ~isempty(paths.targetMaskFile)
                targetMask = niftireg.prepNiiToVol(paths.targetMaskFile, volPad, opts.maxVolSize)  > 0;
            else
        ...
        if dosparse
            if isfield(paths, 'sourceMaskScales') && ~isempty(paths.sourceMaskScales)
                ...
                sourceMask = cellfunc(@(x) niftireg.prepNiiToVol(x, volPad), sourceMaskScales)  > 0;
            else
                warning('No source mask found. Using all-ones');
            ...
            if isfield(paths, 'targetMaskScales') && ~isempty(paths.targetMaskScales)
                ...
                targetMask = cellfunc(@(x) niftireg.prepNiiToVol(x, volPad), targetMaskScales)  > 0;
            else
                warning('No target mask found. Using all-ones');
