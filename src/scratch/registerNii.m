...
% TODO: use cubic interpolation? see if there is a difference?
    ...
    % make and save niis
    displFile = sprintf('%s2%s.nii.gz', paths.sourceName, paths.targetName);
    displInvFile = sprintf('%s2%s_inv.nii.gz', paths.sourceName, paths.targetName);
    sourceWarpedFile = sprintf('%s_in%s_via%s2%s.nii.gz', paths.sourceName, paths.targetName, paths.sourceName, paths.targetName);
    targetWarpedFile = sprintf('%s_in%s_via%s2%s_inv.nii.gz', paths.targetName, paths.sourceName, paths.sourceName, paths.targetName);
    sourceWarpedSegFile = sprintf('%s_seg_in%s_via%s2%s.nii.gz', paths.sourceName, paths.targetName, paths.sourceName, paths.targetName);
    targetWarpedSegFile = sprintf('%s_seg_in%s_via%s2%s_inv.nii.gz', paths.targetName, paths.sourceName, paths.sourceName, paths.targetName);
    
    saveNii(make_nii(displ, displFile), displFile);
    saveNii(make_nii(displInv, displInvFile), displInvFile);
    saveNii(make_nii(sourceWarped, sourceWarpedFile), sourceWarpedFile);
    saveNii(make_nii(targetWarped, targetWarpedFile), targetWarpedFile);
    savenii(make_nii(courceWarpedSeg, sourceWarpedSegFile), sourceWarpedSegFile);
    saveNii(make_nii(targetWarpedSeg, targetWarpedSegFile), targetWarpedSegFile);
