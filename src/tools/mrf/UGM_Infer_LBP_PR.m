function [nodeBel, edgeBel, logZ] = UGM_Infer_LBP_PR(nodePot, curdispl, lambda_edge, edgeStruct)

    if edgeStruct.useMex
        [nodeBel,edgeBel,logZ] = UGM_Infer_LBPC_PR(nodePot,curdispl,lambda_edge, int32(edgeStruct.edgeEnds),int32(edgeStruct.nStates),int32(edgeStruct.V),int32(edgeStruct.E),edgeStruct.maxIter);
    else
        [nodeBel, edgeBel, logZ] = Infer_LBP_PR(nodePot,curdispl, lambda_edge, edgeStruct);
    end

end

function [nodeBel, edgeBel, logZ] = Infer_LBP_PR(nodePot,edgePot,edgeStruct)
    error('unchanged yet. modifications need a''making');
